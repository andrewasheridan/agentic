...
1. **Run checks first** — Look at `CLAUDE.md` to find the project's check command. If a `Taskfile.yml` is present, run `task check`. If a `Makefile` is present, run `make check`. Otherwise run the commands described in CLAUDE.md. If any check fails (lint, format, typecheck, tests), stop immediately and report which check failed with the relevant error output. Do NOT proceed to commit.
